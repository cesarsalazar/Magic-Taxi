...
get '/position/get_closest' do
  ...
  interval = 5
  query_string = "SELECT distinct(`taxi_id`),`created_at` FROM positions WHERE `status`=FALSE AND `created_at` > DATE_SUB(now(), INTERVAL #{interval} MINUTE) AND ((6371*3.1415926*sqrt((`latitude`-#{lat})*(`latitude`-#{lat}) +cos(#{lat}/57.29578)*cos(`lat`/57.29578)*(#{long}-`longitude`)*(#{long}-`longitude`))/180)<3.0)"
  puts query_string
